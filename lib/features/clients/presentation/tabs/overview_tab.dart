...
class _OverviewTabState extends ConsumerState<OverviewTab> {
  ...
  Future<void> _openSetPasswordDialog() async {
    ...
    try {
      await ref.read(clientRepositoryProvider).setLoginPassword(widget.leadId, password);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l10n.loginPasswordSuccess)));
    } on ApiException catch (e) {
      if (mounted) setState(() => _error = e.displayMessage);
    }
  }
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              if (isConverted) ...[
                const Divider(height: 40),
                Text(l10n.loginPasswordTitle, style: const TextStyle(fontWeight: FontWeight.w700)),
                const SizedBox(height: 4),
                Text(l10n.loginPasswordExplain, style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                const SizedBox(height: 8),
                OutlinedButton.icon(
                  icon: Icon(Icons.password, color: MatchmakerTheme.plum),
